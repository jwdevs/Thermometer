...
#
#
# Last update: Oct 20, 2014 release 215
...
#    ifneq ($(strip $(NO_CORE_MAIN_FUNCTION)),)
    s10              = $(filter-out %main.cpp, $(wildcard $(CORE_LIB_PATH)/*.cpp $(CORE_LIB_PATH)/*/*.cpp $(CORE_LIB_PATH)/*/*/*.cpp $(CORE_LIB_PATH)/*/*/*/*.cpp))
# */
#    else
#        CORE_CPP_SRCS = $(wildcard $(CORE_LIB_PATH)/*.cpp $(CORE_LIB_PATH)/*/*.cpp) # */
#    endif
    CORE_CPP_SRCS     = $(filter-out %/$(EXCLUDE_LIST),$(s10))

    CORE_AS1_SRCS_OBJ = $(CORE_AS_SRCS:.S=.S.o)
    CORE_AS2_SRCS_OBJ = $(CORE_AS1_SRCS:.s=.s.o)
...
ifndef APP_LIB_OBJS
    ...
    APP_LIB_OBJS     += $(patsubst $(APP_LIB_PATH)/%.s,$(OBJDIR)/libs/%.s.o,$(APP_LIB_CPP_SRC))
else
    FLAG = 0
endif 
...
#ifneq ($(strip $(NO_CORE_MAIN_FUNCTION)),)
    s9 = $(wildcard $(patsubst %,%/*.cpp,$(LOCAL_LIBS))) $(wildcard $(LOCAL_LIB_PATH)/*.cpp) # */
	LOCAL_CPP_SRCS = $(filter-out %$(PROJECT_NAME_AS_IDENTIFIER).cpp, $(s9))
#else
...
ifeq ($(BOARD_PORT),pgm)
  ...
  ifeq ($(MSPDEBUG_PROTOCOL),tilib)
    ...
    ifneq ($(MAKECMDGOALS),debug)
		...
		$(call TRACE,"10-UPLOAD",$(UPLOADER))
		osascript -e 'tell application "System Events" to display dialog "Press the RESET button on the OpenCM9.04 board and then click OK." buttons {"OK"} with icon POSIX file ("$(UTILITIES_PATH)/TemplateIcon.icns") with title "embedXcode"'
